...
function App() {
  ...
  function Scene() {
    return data.map((data,i) => <Section active={i===counter} key={i} id={i+1}>{data.txt}</Section>);
  }
  ...
  var Body = styled.div`
    ...
    background-image:url(${data[counter].img});
  `;
  ...
  if (welcome===true) {
    return <div className="App">
      ...
      <p>In this page we use React components to show lines of text from an array. You can use the Previous and Next buttons to choose which line is highlighted.</p>
    </div>
  } // Main Page
  else {
    return (
      <Body className="App" id="Body">
        <Buttons/>
        <Scene/>
      </Body>
    )
